...
## Запуск в один клик, без установки

Прогон можно запустить прямо на GitHub, ничего не ставя себе:

**[Actions → API tests → Run workflow](https://github.com/Dilligan1/api-automation-framework/actions/workflows/tests.yml)**

Кнопка открывает форму: выбрать набор тестов (`smoke`, `negative`, `e2e`, `regression`, `critical` или всё сразу), число параллельных процессов и нужно ли сбросить стенд перед прогоном.

Всё выполняется на раннере GitHub: там поднимается тестируемая система в Docker, ставится Python, гоняются тесты и публикуется отчёт. На вашем компьютере не запускается ничего.

> Кнопка «Run workflow» видна только тем, у кого есть права на запись в репозиторий — это ограничение GitHub. Если вы зашли со стороны и хотите запустить прогон, сделайте форк: в нём кнопка заработает сразу, настраивать ничего не нужно — прогон не требует ни секретов, ни внешнего стенда.

Результат виден там же: статус шагов, короткая сводка на странице прогона, архив с результатами и [Allure-отчёт](https://dilligan1.github.io/api-automation-framework/).
